...
PG_SERVICE="openebs-stack-postgresql-primary" # 修改为HA架构中的主节点服务名
PG_PORT=5432
PG_USER="postgres"
PG_PASSWORD="yourpassword"
PG_DATABASE="postgres"
KAFKA_SERVICE="openebs-stack-kafka"
KAFKA_CONNECT_SERVICE="openebs-stack-kafka-connect-0" # 在高可用架构中指定第一个kafka-connect节点
QUICKWIT_SERVICE="openebs-stack-quickwit-0" # 在高可用架构中指定第一个quickwit节点
...
# 等待所有组件准备就绪，为高可用环境增加超时时间
print_section "等待组件准备就绪"

print_step "等待PostgreSQL主节点准备就绪..."
wait_for_pod_ready "openebs-stack-postgresql-primary" "$NAMESPACE" 60 15 || print_error "PostgreSQL主节点未准备就绪"
print_success "PostgreSQL主节点准备就绪"

print_step "等待PostgreSQL读取副本准备就绪..."
wait_for_pod_ready "openebs-stack-postgresql-read" "$NAMESPACE" 60 15 || print_error "PostgreSQL读取副本未准备就绪"
print_success "PostgreSQL读取副本准备就绪"

print_step "等待Kafka控制器准备就绪..."
...
# 获取Kafka Connect URL
# 在高可用环境中，我们直接使用服务名来访问
KAFKA_CONNECT_POD=$(kubectl get pods -n "$NAMESPACE" | grep kafka-connect | head -1 | awk '{print $1}')
KAFKA_CONNECT_URL="http://${KAFKA_CONNECT_POD}:8083"

# 检查并删除已存在的连接器
print_step "检查并删除已存在的Debezium PostgreSQL连接器..."
kubectl exec -it $KAFKA_CONNECT_POD -n "$NAMESPACE" -- curl -s -X DELETE "http://localhost:8083/connectors/postgres-source" || echo "没有找到现有连接器或删除失败，继续创建..."
...
# 构建PostgreSQL连接器配置（添加了ExtractNewRecordState转换器，并为高可用环境增加复制因子）
...
# 使用直接exec到pod中创建连接器
echo "$PG_CONNECTOR_CONFIG" > /tmp/pg_connector.json
kubectl cp /tmp/pg_connector.json "$NAMESPACE/$KAFKA_CONNECT_POD:/tmp/pg_connector.json"
kubectl exec -it $KAFKA_CONNECT_POD -n "$NAMESPACE" -- curl -X POST -H "Content-Type: application/json" \
  http://localhost:8083/connectors -d @/tmp/pg_connector.json || print_error "创建Debezium PostgreSQL连接器失败"
...
# 获取Quickwit pod
QUICKWIT_POD=$(kubectl get pods -n "$NAMESPACE" | grep quickwit | head -1 | awk '{print $1}')
...
# 配置成功
print_section "配置完成"
echo -e "\033[1;32m高可用数据流配置已完成: PostgreSQL -> Kafka Connect -> Kafka -> Quickwit\033[0m"
...
echo -e "\nQuickwit查询示例:"
echo "# 使用CLI查询文本内容(推荐):"
echo "kubectl exec -it \$(kubectl get pods | grep quickwit | head -1 | awk '{print \$1}') -- quickwit index search --index messages --query \"message:测试\" --max-hits 10"
echo ""
echo "# 或使用REST API查询:"
echo "kubectl exec -it \$(kubectl get pods | grep quickwit | head -1 | awk '{print \$1}') -- curl -s -X POST 'http://localhost:7280/api/v1/messages/search' -H 'Content-Type: application/json' -d '{\"query\": \"message:测试\", \"max_hits\": 5}'" 